...
local background_img = sol.surface.create("hud/Y_item.png")

function item_icon_builder:new(game, config)

  local item_icon = {}

  item_icon.slot = config.slot or 2
  item_icon.surface = sol.surface.create(24, 24)
  ...
  function item_icon:rebuild_surface()
    ...
    if item_icon.item_displayed ~= nil then
      -- Item.
      item_icon.item_sprite:draw(item_icon.surface, 3 + 9, 3 + 14)
    end
