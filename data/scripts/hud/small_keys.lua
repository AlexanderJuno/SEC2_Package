...
function small_keys_builder:new(game, config)
  ...
  small_keys.surface = sol.surface.create(44, 25)
  small_keys.icon_img = sol.surface.create("hud/small_key.png")
...
function small_keys:check()
    ...
    if game:are_small_keys_enabled() then
      ...
      if nb_keys_displayed ~= nb_keys then
        small_keys.digits_text:set_text(string.format("%02d", nb_keys)) -- syntaxe cause une erreur
        need_rebuild = true
      end
    end
  ...
  function small_keys:rebuild_surface()
    ...
    small_keys.icon_img:draw(small_keys.surface, 3, 0)
    small_keys.digits_text:draw(small_keys.surface, 18, 4)
  end
